...
module.exports = {
            ...
            if(user.length === 1) {
                ...
                .then((passwordMatch) => {
                    if (!passwordMatch) {
                        ...
                        res.status(200).send({message: `User with id: ${user[0].id} loggedin successfully`})
                    }
                })
            }
    ...
    signup(req, res) {
        db.User.findAll({
            ...
            if (user.length !== 0) {
                ...
                bcryptjs.hash(req.body.password, BCRYPT_WORK_FACTOR)
                .then((hashedPassword) => {
                    db.User.create({
                        ...
                        res.status(201).send({message: `User with id ${newUser.id} created successfully`})})
                })
            }
        })
        .catch((error) => {
    ...
    delete(req,res) {
        if( !req.session.user.id || req.params.id != req.session.user.id) {
            res.status(401).send({ "message": "Unathorized to delete user"})
        }
        else {
            db.User.destroy({
            ...
            .then((result) => {
                if (result)
                    ...
                    res.status(404).send({ "error": "User deletion failed"})
            })
            .catch(error => res.status(404).send({ "error": "Something went wrong, try again"}))
        }
    }
}